...
window.addEventListener('online', () => {
    ...
    if (currentVideoId && player && syncRef) {
...
window.onYouTubeIframeAPIReady = function() {
    player = new YT.Player('player', {
        ...
        playerVars: { 
            ...
            'playsinline': 1,
            'iv_load_policy': 3 // 3 = Disable video annotations (No cards/carts)
        },
        events: { 
...
function onPlayerReady(event) {
    ...
    // SMART TIMER: Monitor Sync Health (1s active, 5s hidden) - Checks every second
...
function monitorAdStatus() {
    ...
    if (isAd) {
        if (!wasInAd) {
            ...
            // AGGRESSIVE AD SKIP ATTEMPT
            try {
                if(player.mute) player.mute(); // Mute ad
                if(player.setPlaybackRate) player.setPlaybackRate(2); // Speed up ad
            } catch(e) {}
        }
    } else {
        if (wasInAd) {
...
function heartbeatSync() {
    ...
    // Only the "Broadcaster" (person who last touched controls) updates the DB
...
// --- PRECISION SYNC CHECKER ---
// Checks every second if we are drifting from the group
function monitorSyncHealth() {
    if (!hasUserInteracted) return;
    // Don't sync if I am the one broadcasting or we are switching songs
    if (lastBroadcaster === myName || isSwitchingSong) return;
    if (!player || !currentRemoteState || !player.getPlayerState) return;
    // Don't sync if I just clicked something myself
    ...
    if (currentRemoteState.action === 'play' || currentRemoteState.action === 'restart') {
        ...
        // 2. Check Time Drift with LATENCY COMPENSATION
        const now = Date.now();
        const msgAge = now - currentRemoteState.timestamp; // How long ago the message was sent (ms)
        const latencySec = msgAge / 1000;
        
        // The song *should* be at: recorded_time + elapsed_time_since_record
        const targetTime = currentRemoteState.time + latencySec;
        const myTime = player.getCurrentTime();
        
        // DRIFT THRESHOLD: 1.5 Seconds (Strict Sync)
        if (Math.abs(myTime - targetTime) > 1.5) {
            if (!detectAd()) { 
                console.log(`Syncing: Drift detected. Mine: ${myTime}, Target: ${targetTime}`);
                player.seekTo(targetTime, true); 
                needsFix = true; 
            }
        }
        
        // If we corrected, suppress our own broadcasts for a bit so we don't cause a loop
...
function loadInitialData() {
    if (!queueRef || !syncRef || !chatRef) return;
...
function isChatActive() {
    ...
    if (isMobile) {
        return activeTab === 'chat' && UI.mobileSheet && UI.mobileSheet.classList.contains('active');
    }
...
if(document.getElementById('mobileSheetClose')) {
...
function addBatchToQueue(songs) {
    if (!songs.length || !queueRef) return;
...
if(document.getElementById('lyrics-btn')) {
    document.getElementById('lyrics-btn').addEventListener('click', () => {
        if(UI.lyricsOverlay) UI.lyricsOverlay.classList.add('active');
        fetchLyrics();
    });
}
if(document.getElementById('closeLyricsBtn')) {
    document.getElementById('closeLyricsBtn').addEventListener('click', () => {
        if(UI.lyricsOverlay) UI.lyricsOverlay.classList.remove('active');
        stopLyricsSync(); 
    });
}

if(document.getElementById('manualLyricsBtn')) {
...
if(document.getElementById('manualLyricsInput')) {
    document.getElementById('manualLyricsInput').addEventListener('keydown', (e) => {
...
if (dedicateBtn) {
    dedicateBtn.addEventListener('click', () => {
        ...
        if(title && title !== "Heart's Rhythm") {
            ...
            if(chatRef) chatRef.push({ user: myName, text: msg, timestamp: Date.now(), seen: false });
...
function sendVibe(emoji) {
    ...
    if(chatRef) chatRef.push({ user: myName, text: text, timestamp: Date.now(), seen: false });
    triggerHaptic();
}
...
function syncLyricsDisplay() {
    if (document.hidden) return;
    if (!player || typeof player.getCurrentTime !== 'function' || !currentLyrics) return;
            ...
            try {
                ...
                if(titleText.includes('-')) {
                   ...
                   if(fData.lyrics) {
                        currentLyrics = null;
                        stopLyricsSync();
                        if(UI.lyricsContent) UI.lyricsContent.innerHTML = `<div class="lyrics-text-block" style="text-align:center;">${fData.lyrics.replace(/\n/g, "<br>")}</div>`;
                        return; 
                   }
...
async function handleSearch() {
    ...
    if (ytPlaylistMatch) {
        ...
        await fetchPlaylist(ytPlaylistMatch[1]);
        input.value = ''; 
        return;
    }
    ...
    // REGULAR SEARCH
    switchTab('results', true);
    if(UI.resultsList) UI.resultsList.innerHTML = '<p style="text-align:center; padding:30px; color:white;">Searching...</p>';
...
async function fetchPlaylist(playlistId, pageToken = '', allSongs = []) {
    ...
    try {
        ...
        const songs = items.filter(i=>i.snippet.resourceId.kind==='youtube#video').map(i => ({
        ...
        if (data.nextPageToken) {
            await fetchPlaylist(playlistId, data.nextPageToken, allSongs);
        } else {
