...
// --- USER IDENTIFICATION (FIXED NAMES) ---
let myName = "Sarthak"; 
...
// Ensure DB paths are consistently Sarthak/Reechita regardless of input casing
// We will use this FIXED name for all internal sync logic (lastUpdater, myRef, etc.)
...
// FIX 1: This is the critical chat path we must write to and listen to.
const partnerChatRef = db.ref(`users/${fixedPartnerName}/chat`);

const partnerDisplayLabel = fixedPartnerName;


// --- GLOBAL STATE (FIX 2: Added lastLocalActionTime) ---
let player; 
let currentQueue = [];
...
let lastLocalActionTime = 0; // NEW: Timestamp of the last local play/pause action
...
function broadcastTimeIfPlaying() {
    if (player && player.getPlayerState && currentVideoId) {
        ...
        // Only broadcast if we are currently playing AND we were the one who initiated play (using fixedDBName)
        if (state === YT.PlayerState.PLAYING && lastBroadcaster === fixedDBName) {
...
function onPlayerStateChange(event) {
    ...
    if (event.data === YT.PlayerState.PLAYING) {
        ...
        if (event.data === YT.PlayerState.PAUSED || event.data === YT.PlayerState.BUFFERING) {
            ...
            // Only broadcast Ad Stall if paused unexpectedly (not manually, not due to partner)
            if (timeSinceLastAction > 500 && !isPartnerPlaying && lastBroadcaster === fixedDBName) {
...
function togglePlayPause() {
    ...
    // FIX 2: Record the time of the local manual action and set broadcaster
    ...
    if (state === YT.PlayerState.PLAYING || state === YT.PlayerState.BUFFERING) {
        ...
        // CRITICAL FIX: Broadcast PAUSE immediately
        broadcastState('pause', player.getCurrentTime(), currentVideoId, false); 
        ...
        if (!currentVideoId && currentQueue.length > 0) {
            ...
            // CRITICAL FIX: Broadcast PLAY immediately
            broadcastState('play', player.getCurrentTime(), currentVideoId, false);
        }
    }
}
...
async function handleSearchAndLinks() {
    ...
    if (!searchStatusEl) {
        ...
        resultsList.insertBefore(searchStatusEl, resultsList.querySelector('.search-container').nextSibling);
    }
...
// ------------------------------------------------------------------------------------------------------
// --- FIREBASE SYNC (CRITICAL FIXES APPLIED) ---
// ------------------------------------------------------------------------------------------------------
...
function loadInitialData() {
    ...
    partnerSyncRef.on('value', (snapshot) => {
        ...
        if (syncState) {
            // FIX 3: Ensure we are using the fixedDBName for internal comparisons
    ...
    // Listen to PARTNER's chat path for their messages (partnerChatRef)
    partnerChatRef.limitToLast(50).on('child_added', (snapshot) => {
        const message = snapshot.val();
        // Skip messages sent by me (to prevent double-display if I write to both refs)
...
function broadcastState(action, time, videoId = currentVideoId, isAdStall = false) {
    ...
    mySyncRef.set({
        ...
        time: time,
        videoId: videoId,
        lastUpdater: fixedDBName, // FIX 3: Use fixedDBName as the consistent identifier
        isAdStall: isAdStall, 
        timestamp: Date.now()
    });
}
...
function applyRemoteCommand(state) {
    ...
    isPartnerPlaying = true; // Mark that the partner is currently dictating state
    const localTime = player.getCurrentTime ? player.getCurrentTime() : 0; 
    
    // FIX 2: Ignore remote signal if we just performed a manual action (within 500ms)
    ...
    // SCENARIO 1: PARTNER IS STALLED (Ad/Buffer)
...
function forcePlay() {
    document.getElementById('syncOverlay').classList.remove('active');
    lastBroadcaster = fixedDBName; // Force me to be the broadcaster
    lastLocalActionTime = Date.now();
    if (currentVideoId) {
        player.playVideo();
        broadcastState('play', player.getCurrentTime(), currentVideoId, false);
...
function updateSyncStatus() {
    ...
    // Use partnerDisplayLabel for better user experience
    const broadcasterName = (lastBroadcaster === fixedDBName) ? myName : partnerDisplayLabel; 
    
    if (overlayIsActive) {
           document.getElementById('overlayTitle').textContent = `⚠️ PAUSED by ${broadcasterName}`;
           document.getElementById('overlayText').innerHTML = `Playback paused because **${broadcasterName}** is experiencing an **Ad/Buffer stall**. Please wait for them to resume or use **Force Play & Sync** to override.`;
           
           msgEl.innerHTML = `<i class="fa-solid fa-triangle-exclamation pulse"></i> **STALL HALT** (Waiting for ${broadcasterName})`;
...
function sendChatMessage() {
    ...
    const timestamp = Date.now(); // Using local time for simplicity
    ...
    // FIX 1 (CRITICAL): Write to the PARTNER's chat path so they receive it.
